...
public:
  ...
  // Neither required nor defaulted: has() says whether it was given.
  template <typename T> Schema& optional(std::string key, std::string doc);
  ...
  // A key=value file, such as an interpolator's; no positional argument
  Params parse_file(const std::string& path) const;
...
private:
  ...
  Params resolve(Params p, std::vector<std::string> problems) const;
...
template <typename T>
Schema& Schema::optional(std::string key, std::string doc) {
  detail::Entry e;
  e.key = std::move(key);
  e.kind = detail::kind_of<T>();
  e.doc = std::move(doc);
