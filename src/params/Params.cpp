...
Params Schema::parse(const std::vector<std::string>& args) const {
  ...
  return resolve(std::move(p), std::move(problems));
}

// Required keys, finalizers, choices, checks and warnings
Params Schema::resolve(Params p, std::vector<std::string> problems) const {
...
Params Schema::parse_file(const std::string& path) const {
  std::vector<std::string> problems;
  Params p;
  p.m_schema = m_impl;
  ...
  std::ifstream in(path);
  if (!in)
    throw ParamError(m_impl->app, {"cannot open parameter file '" + path + "'"});
  ...
  for (std::string line; std::getline(in, line);) {
    ...
    if (eq == std::string::npos) {
      problems.push_back(path + ":" + std::to_string(lineno) + ": '" + t +
                         "' is not key=value");
      continue;
    }
    ...
    if (given.count(key))
      problems.push_back("parameter '" + key + "' given more than once in " + path);
    given[key] = trim(t.substr(eq + 1));
  }

  for (const auto& kv : given) {
    const detail::Entry* e = m_impl->find(kv.first);
    if (!e) {
      const std::string sug = suggest(kv.first, m_impl->entries);
      problems.push_back("unknown parameter '" + kv.first + "' in " + path +
  ...
  for (const auto& kv : given) {
    const detail::Entry* e = m_impl->find(kv.first);
    if (!e) continue;  // already reported
    Value v;
    std::string err;
    if (!convert(*e, kv.second, v, err)) problems.push_back(kv.first + ": " + err + " (in " + path + ")");
    else p.store(kv.first, std::move(v), Source::File);
  }

  return resolve(std::move(p), std::move(problems));
}

bool Schema::declares(const std::string& key) const {
  return m_impl->find(key) != nullptr;
}
