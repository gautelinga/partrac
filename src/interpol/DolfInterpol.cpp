...
DolfInterpol::DolfInterpol(const std::string& infilename) : Interpol(infilename) {
  ...
  //std::cout << dolfin_params.get<std::string>("velocity") << std::endl;

  //std::string xdmffname = get_folder() + "/" + dolfin_params.get<std::string>("velocity");
  ...
  const std::vector<std::uint32_t> order =
    cell_order(*u_space->dofmap(), ncells, dolfin_params.get<std::string>("renumber_cells"), dolfin2local_);
