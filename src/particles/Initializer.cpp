...
void set_initial_state(std::shared_ptr<Initializer>& init_state, std::shared_ptr<Interpol> intp, partrac::Params& prm, std::mt19937& gen){
  ...
  if (key.size() == 0){
    ...
    init_state = std::make_shared<UniformInitializer>(key, intp, prm);
  }
  else if (key[0] == "strip"){
    init_state = std::make_shared<StripInitializer>(key, intp, prm);
  }
  else if (key[0] == "sheet"){
    init_state = std::make_shared<SheetInitializer>(key, intp, prm);
  }
  else if (key[0] == "ellipsoid"){
    init_state = std::make_shared<EllipsoidInitializer>(key, intp, prm);
  }
  else if (key[0] == "pair" || key[0] == "pairs"){
    init_state = std::make_shared<RandomPairsInitializer>(key, intp, prm, gen);
  }
  else if (key[0] == "points"){
    init_state = std::make_shared<RandomPointsInitializer>(key, intp, prm, gen);
  }
  else if (key[0] == "randomgaussianstrip"){
    init_state = std::make_shared<RandomGaussianStripInitializer>(key, intp, prm, gen);
  }
...
std::shared_ptr<Initializer> make_gaussian_initializer(const Uint dim, const std::vector<std::string>& key, std::shared_ptr<Interpol> intp, partrac::Params& prm, std::mt19937& gen){
  if (dim == 2)
    return std::make_shared<RandomGaussianStripInitializer>(key, intp, prm, gen);
  return std::make_shared<RandomGaussianCircleInitializer>(key, intp, prm, gen);
}
